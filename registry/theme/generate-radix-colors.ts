// @ts-nocheck — vendored third-party file (Radix); not held to this repo's strict
// TS config. Edit upstream and re-vendor rather than patching here.
/**
 * generateRadixColors — vendored from radix-ui/website (MIT).
 * https://github.com/radix-ui/website/blob/main/components/generate-radix-colors.tsx
 *
 * This is the engine behind https://www.radix-ui.com/colors/custom : given a brand
 * `accent`, a `gray`, and a page `background` (all hex), it bends the closest Radix
 * reference scale toward your brand and returns a full 12-step Radix scale —
 * solid + alpha, plus `accentContrast` (legible text on step 9) and surface fills.
 * Call once per `appearance` ("light" | "dark").
 *
 * Vendored (not an npm dependency) because Radix never published it standalone.
 * Pure TS — no JSX. Deps: @radix-ui/colors, colorjs.io, bezier-easing.
