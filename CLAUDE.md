...
The contract in `default.css` follows [Radix Colors](https://www.radix-ui.com/colors/docs/palette-composition/understanding-the-scale), **not** the shadcn `--primary`/`--secondary`/`--muted` set. Three 12-step scales — `gray` (neutral chrome), `accent` (brand), `danger` (destructive) — each with a solid ramp (`--{scale}-1`…`-12`) and an alpha ramp (`--{scale}-a1`…`-a12`), in `:root` (light) + `.dark`. Steps carry fixed roles: `1` app bg, `2` subtle bg, `3-5` component bg/hover/active, `6-8` borders/focus, `9` solid (the pure color), `10` solid-hover, `11` low-contrast text, `12` high-contrast text. Specials: `--accent-contrast`/`--danger-contrast` (text on step 9), `--{scale}-surface` (translucent panels), `--background`, `--overlay`. `@theme inline` exposes everything as Tailwind utilities (`bg-accent-9`, `text-gray-11`, `border-gray-6`, `bg-accent-a3`, `text-accent-contrast`). Components stay unstyled (empty slots); the contract is the palette consumers fill in.
